...
export function ManageMonitors() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const mins = Number(form.intervalMins);
    if (!Number.isFinite(mins) || mins < 1) {
      setError("Check interval must be at least 1 minute.");
      return;
    }
    setSaving(true); setError(null);
    const payload = { name: form.name.trim(), url: form.url.trim(), intervalMins: mins, isActive: form.isActive };
    ...
    <div className="dashboard-shell min-h-screen">
        ...
        <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h1 className="text-xl font-semibold tracking-tight text-[var(--text-main)]">Monitor management</h1>
            </div>
          </div>
      ...
      <main className="mx-auto max-w-7xl px-5 py-7 md:px-8">
        <div className="mb-7 flex flex-wrap items-end justify-between gap-4">
          <div>
            <h2 className="text-2xl font-semibold tracking-tight text-[var(--text-main)]">Monitors</h2>
            <p className="mt-1 text-sm text-[var(--text-muted)]">Configure endpoints, review their latest checks, and control monitoring.</p>
          </div>
          <button type="button" onClick={startCreate} className="management-button management-button-primary">+ Add monitor</button>
        </div>
        ...
        {formOpen && <section className="dashboard-panel mb-6 p-5"><div className="flex items-center justify-between gap-4"><div><h3 className="text-base font-semibold text-[var(--text-main)]">{editingId ? "Edit monitor" : "Add monitor"}</h3><p className="mt-1 text-xs text-[var(--text-muted)]">{editingId ? "Update the endpoint or monitoring configuration." : "Add a URL to begin monitoring its availability."}</p></div><button type="button" onClick={resetForm} className="text-xs text-[var(--text-muted)] hover:text-[var(--text-main)]">Close</button></div><form onSubmit={handleSubmit} className="mt-5 grid gap-4 md:grid-cols-2"><div><label htmlFor="name" className="management-label">Monitor name</label><input id="name" required value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} className="management-control w-full" placeholder="Production API" /></div><div><label htmlFor="url" className="management-label">URL</label><input id="url" type="url" required value={form.url} onChange={(event) => setForm({ ...form, url: event.target.value })} className="management-control w-full" placeholder="https://api.example.com/health" /></div><div><label htmlFor="intervalMins" className="management-label">Check interval (minutes)</label><input id="intervalMins" type="number" min={1} required value={form.intervalMins} onChange={(event) => setForm({ ...form, intervalMins: event.target.value })} className="management-control w-full" /></div>{editingId && <label className="flex items-center gap-2 self-end pb-2 text-sm text-[var(--text-main)]"><input type="checkbox" checked={form.isActive} onChange={(event) => setForm({ ...form, isActive: event.target.checked })} className="h-4 w-4 rounded border-[var(--color-grid)] bg-bg-strip text-phosphor" /> Monitoring enabled</label>}<div className="flex gap-3 md:col-span-2"><button type="submit" disabled={saving} className="management-button management-button-primary">{saving ? "Saving…" : editingId ? "Save changes" : "Add monitor"}</button><button type="button" onClick={resetForm} className="management-button management-button-secondary">Cancel</button></div></form></section>}

        <section className="dashboard-panel overflow-hidden"><div className="flex flex-wrap items-center justify-between gap-4 border-b border-grid p-4 md:p-5"><div><h3 className="text-base font-semibold text-[var(--text-main)]">All monitors <span className="ml-1 text-sm font-normal text-[var(--text-muted)]">{monitors.length}</span></h3><p className="mt-1 text-xs text-[var(--text-muted)]">Search, filter, and manage your monitored services.</p></div><div className="flex flex-wrap gap-2"><div className="relative"><input value={search} onChange={(event) => setSearch(event.target.value)} className="management-control w-56 pr-8" placeholder="Search monitors…" aria-label="Search monitors" /><span className="pointer-events-none absolute right-3 top-2.5 text-[var(--text-muted)]">⌕</span></div>{(["all", "up", "down", "paused"] as Filter[]).map((value) => <button key={value} type="button" className="dashboard-filter" aria-pressed={filter === value} onClick={() => setFilter(value)}>{value[0].toUpperCase() + value.slice(1)}</button>)}</div></div>
          {loading ? <div className="p-8"><p className="text-sm text-[var(--text-muted)]">Loading monitors…</p><div className="mt-4 space-y-3">{Array.from({ length: 3 }, (_, index) => <div key={index} className="h-16 animate-pulse rounded-lg bg-bg-strip" />)}</div></div> : monitors.length === 0 ? <div className="p-12 text-center"><div className="mx-auto flex h-11 w-11 items-center justify-center rounded-full bg-[rgba(var(--color-phosphor-rgb),0.1)] text-xl text-phosphor">+</div><h3 className="mt-4 text-base font-semibold text-[var(--text-main)]">No monitors yet</h3><p className="mx-auto mt-2 max-w-sm text-sm text-[var(--text-muted)]">Add your first endpoint to start tracking uptime and response time.</p><button type="button" onClick={startCreate} className="management-button management-button-primary mt-5">Add your first monitor</button></div> : filteredMonitors.length === 0 ? <div className="p-12 text-center"><h3 className="text-base font-semibold text-[var(--text-main)]">No matching monitors</h3><p className="mt-2 text-sm text-[var(--text-muted)]">Try another search term or clear the active filter.</p><button type="button" onClick={() => { setSearch(""); setFilter("all"); }} className="mt-4 text-xs font-medium text-phosphor hover:underline">Clear filters</button></div> : <div>{filteredMonitors.map((monitor) => <Fragment key={monitor.id}><article className="management-table-row grid gap-4 p-4 md:grid-cols-[minmax(14rem,1.4fr)_minmax(10rem,1fr)_7rem_9rem_auto] md:items-center md:p-5"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h4 className="truncate text-sm font-semibold text-[var(--text-main)]">{monitor.name}</h4><StatusBadge monitor={monitor} /></div><p className="mt-1 truncate text-xs text-[var(--text-muted)]">{monitor.url}</p></div><div className="grid grid-cols-2 gap-3 text-xs"><div><p className="text-[var(--text-muted)]">Response time</p><p className="mt-1 font-medium text-[var(--text-main)]">{monitor.responseTimeMs == null ? "—" : `${monitor.responseTimeMs} ms`}</p></div><div><p className="text-[var(--text-muted)]">Last check</p><p className="mt-1 font-medium text-[var(--text-main)]">{formatCheckedAt(monitor.checkedAt)}</p></div></div><div className="text-xs"><p className="text-[var(--text-muted)]">Interval</p><p className="mt-1 font-medium text-[var(--text-main)]">Every {monitor.intervalMins} min</p></div><div className="text-xs"><p className="text-[var(--text-muted)]">Check state</p><p className="mt-1 font-medium text-[var(--text-main)]">{monitor.checkedAt ? `${monitor.statusCode ?? "—"} · ${monitor.status.toUpperCase()}` : "Pending"}</p></div><div className="flex flex-wrap gap-2 md:justify-end"><button type="button" disabled={saving} onClick={() => handleToggle(monitor)} className="management-button management-button-secondary">{monitor.isActive ? "Disable" : "Enable"}</button><button type="button" onClick={() => startEdit(monitor)} className="management-button management-button-secondary">Edit</button><button type="button" disabled={saving} onClick={() => setPendingDelete(monitor)} className="management-button management-button-danger">Delete</button></div></article><AssertionManager monitorId={monitor.id} monitorName={monitor.name} /></Fragment>)}</div>}
        </section>

        <section className="mt-6"><PublicSlugSettings /></section>
      </main>

      {pendingDelete && <div className="confirmation-backdrop fixed inset-0 z-50 flex items-center justify-center p-5" role="presentation"><section role="dialog" aria-modal="true" aria-labelledby="delete-title" className="dashboard-panel w-full max-w-md p-6 shadow-2xl"><h2 id="delete-title" className="text-lg font-semibold text-[var(--text-main)]">Delete monitor?</h2><p className="mt-2 text-sm leading-relaxed text-[var(--text-muted)]">This will permanently remove <span className="font-medium text-[var(--text-main)]">{pendingDelete.name}</span> and its recorded checks. This action cannot be undone.</p><div className="mt-6 flex justify-end gap-3"><button type="button" disabled={saving} onClick={() => setPendingDelete(null)} className="management-button management-button-secondary">Cancel</button><button type="button" disabled={saving} onClick={handleDelete} className="management-button management-button-danger">{saving ? "Deleting…" : "Delete monitor"}</button></div></section></div>}
    </div>
  );
}
