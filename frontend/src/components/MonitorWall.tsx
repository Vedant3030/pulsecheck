...
const FILTERS: Filter[] = ["all", "up", "down", "paused"];
...
export function MonitorWall() {
  const { monitors, loading, error, lastUpdated, refetch } = useMonitors({
     ...
     <div className="dashboard-shell flex bulb-pulse-wave animate relative">
        ...
        <aside className="dashboard-sidebar fixed inset-y-0 left-0 z-20 flex flex-col p-4">
         ...
         <div className="dashboard-panel mt-auto p-3">
           ...
           <p className="mt-1 text-xs leading-relaxed text-[var(--text-muted)]">{hasIncident ? "One or more active monitors are down." : "All active checks are healthy."}</p>
         </div>
       </aside>

<div className="min-w-0 flex-1 lg:ml-[15.5rem] phosphor-grid-pulse animate">
         <header className="sticky top-0 z-10 border-b border-grid bg-bg/95 px-5 py-4 backdrop-blur md:px-8">
           <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-4">
             <div className="flex items-center gap-3">
               <span className="rounded-md border border-grid px-2 py-1 text-xs text-[var(--text-muted)] lg:hidden">⌁</span>
               <div>
                 <h1 className="text-xl font-semibold tracking-tight text-[var(--text-main)]">Overview</h1>
                 <p className="mt-0.5 text-xs text-[var(--text-muted)]">Real-time health and performance across your services</p>
               </div>
             </div>
             <div className="flex items-center gap-4">
               <div className="hidden text-right sm:block"><LiveClock /></div>
               <div className="hidden h-8 w-px bg-grid sm:block" />
               <div className="text-right text-xs">
                 <p className="text-[var(--text-main)]">{getEmail() ?? "operator"}</p>
                 <p className="mt-1 text-[var(--text-muted)]">{lastUpdated ? `Updated ${lastUpdated.toLocaleTimeString("en-US", { hour12: false })}` : "Connecting…"}</p>
               </div>
               <ThemeToggle />
               <Link href="/manage" className="rounded-md border border-grid px-3 py-2 text-xs font-medium text-[var(--text-main)] transition hover:border-phosphor hover:bg-bg-strip">Manage</Link>
...
<main className="mx-auto max-w-7xl px-5 py-6 md:px-8">
           ...
           {loading ? <MonitorBaySkeleton count={4} /> : monitors.length === 0 ? <MonitorBayEmpty /> : <>
             <section aria-label="System health summary" className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
               ...
               <div className="dashboard-stat"><p className="dashboard-stat-label">Total monitors</p><p className="dashboard-stat-value mt-3">{monitors.length}</p><p className="mt-2 text-xs text-[var(--text-muted)]">{summary.activeCount} active</p></div>
               <div className="dashboard-stat"><p className="dashboard-stat-label">Active monitors</p><p className="dashboard-stat-value mt-3">{summary.upCount} <span className="text-base font-medium text-[var(--text-muted)]">/ {summary.activeCount}</span></p><p className={"mt-2 text-xs " + (summary.downCount ? "text-alarm" : "text-phosphor")}>{summary.downCount ? summary.downCount + " down" : "No active incidents"}</p></div>
               <div className="dashboard-stat"><p className="dashboard-stat-label">Average latency</p><p className="dashboard-stat-value mt-3">{summary.averageLatency == null ? "—" : `${summary.averageLatency} ms`}</p><p className="mt-2 text-xs text-[var(--text-muted)]">Across healthy monitors</p></div>
               <div className="dashboard-stat"><p className="dashboard-stat-label">Current uptime</p><p className="dashboard-stat-value mt-3">{summary.availability == null ? "—" : `${summary.availability}%`}</p><p className="mt-2 text-xs text-[var(--text-muted)]">Based on latest checks</p></div>
             </section>
